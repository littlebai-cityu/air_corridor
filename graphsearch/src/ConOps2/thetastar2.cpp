...
bool ThetaStar2::line_of_sight(const Cell *from, const Cell *to, double *coeff) {
...
double ThetaStar2::get_traversal_cost(const Cell *from, const Cell *to) {
    double coeff = 1.0;
...
double ThetaStar2::get_heuristic(const Cell *a) {
    if (nonuniform) {
...
void ThetaStar2::compute_cost(Cell *c, Cell *adj) {
    if (c->parent) {
    ...
    AStar_conops2::compute_cost(c, adj);
}
