...
class AStar_conops2 {
public:
    // Constructor of the AStar_conops2 class.
    // Parameters:
    ...
    AStar_conops2(const array_t<int32_t> & cells_type, const array_t<double> & cells_cost, const array_t<double> & xs, const array_t<double> & ys, const array_t<double> & zs, 
    ...
    // Destructor of the AStar_conops2 class.
    virtual ~AStar_conops2(){};
    ...
    // Add attribute of direction check to cell
    // Parameters:
    // - from_i/from_j/from_k: current one
    // - to_i/to_j/to_k: next one
    void add_route_seg(int64_t from_i, int64_t from_j, int64_t from_k, int64_t to_i,int64_t to_j,int64_t to_k){
        Cell & c = cells[index(from_i, from_j, from_k)];        //add to node for current node
        int to_index = index(to_i, to_j, to_k);
        if(c.to_cell_indexes.empty()){
            c.to_cell_indexes.push_back(to_index);
        }else{
            bool dif=true;
            for(auto &ind:c.to_cell_indexes){
                if(ind==to_index){dif=false;break;}             //avoid repeat record
            }
            if(dif==true)c.to_cell_indexes.push_back(to_index);
        }
        Cell & c2 = cells[index(to_i, to_j, to_k)];//add from node for next node
        int from_index = index(from_i, from_j, from_k);
        if(c2.from_cell_indexes.empty()){
            c2.from_cell_indexes.push_back(from_index);
        }else{
            bool dif=true;
            for(auto &ind:c2.from_cell_indexes){
                if(ind==from_index){dif=false;break;}//avoid repeat record
            }
            if(dif==true)c2.from_cell_indexes.push_back(from_index);
        }
    }
    ...
    // Conflicts arise when a cell is used by path-path or path-buffer
...
void backtrack_cell(const Route& p, const int& id,AStar_conops2 & solver, const int& space_length, const int& xlim, const int& ylim,
...
void write_cells(AStar_conops2& solver,const std::vector<Agent>& route_parallel,const int& space_length,
const int& xlim, const int& ylim, const int& zlim, const double& grid_res);//, const int& ki);
}
