...
namespace conop1{
AStar_conops1::AStar_conops1(const array_t<int32_t> & cells_type, const array_t<double> & cells_cost, const array_t<double> & xs, const array_t<double> & ys, 
    ...
    climb_threshold(climb_threshold), space_cost_coef(space_ccoef), space_length(space_length), grid_res(grid_res), nonuniform(nonuniform), operational(operational)
{
    ...
    // std::cout << "grid_res: " << grid_res << std::endl;
...
void AStar_conops1::init_cells(const double *xs, const double *ys, const double *zs, const int32_t *cells_type, const double *cells_cost) {
    ...
    for (int i = 0; i < grid_size[0]; i++) {
        for (int j = 0; j < grid_size[1]; j++) {
            for (int k = 0; k < grid_size[2]; k++) {
                ...
                // Update the minimum traversal cost
...
int AStar_conops1::conflict_size(){
    int times_path = 0;
    int conflict_size = 0;
    double conflict_threshold = 5.0; // 10 meters
    std::set<std::pair<int, int>> processed_pairs;
    std::unordered_set<const Cell*> conflict_cells; // 修改为 const Cell*

    // 收集所有路径点
    std::vector<std::tuple<const Cell*, int, route_occupation>> all_paths; // 添加时间维度
    for (auto &c : cells) {
        for (route_occupation occupied : c.occupied_routes) {
            if (!occupied.protected_) { // 如果路径点不是缓冲区
                all_paths.push_back(std::make_tuple(&c, c.l, occupied)); // 假设 c.l 代表时间
            }
        }
    }

    // 检查每对路径点之间的距离
    for (size_t i = 0; i < all_paths.size() - 1; ++i) {
        for (size_t j = i + 1; j < all_paths.size(); ++j) {
            const auto& [cell1, l1, route1] = all_paths[i];
            const auto& [cell2, l2, route2] = all_paths[j];
            if (route1.route_id == route2.route_id) {
                continue; // 跳过同一路径的点
            }
            std::pair<int, int> route_pair = std::minmax(route1.route_id, route2.route_id);
            if (processed_pairs.find(route_pair) != processed_pairs.end()) {
                continue; // 跳过已处理过的路径对
            }
            double distance = calculate_cell_distance(cell1->i, cell1->j, cell1->k, cell2->i, cell2->j, cell2->k, grid_res);
            if (distance < conflict_threshold && l1 == l2 && conflict_cells.find(cell1) == conflict_cells.end()) { // 同一时间步长的点才会冲突
                processed_pairs.insert(route_pair);
                conflict_cells.insert(cell1);
                // conflict_cells.insert(cell2);
                times_path++;
            }
        }
    }
    ...
    if (times_path > 0) {
        conflict_size += times_path * 3; // path-path conflicts * 3
    }
    ...
    for (auto &c : cells) {
        ...
        if (c.occupied_routes.size() > 1) {
            int times_path = 0;
            int times_buffer = 0;
            std::vector<route_occupation> conflicting_routes;
            for (route_occupation occupied : c.occupied_routes) {
                if (!occupied.protected_ && conflict_cells.find(&c) == conflict_cells.end()) { // unless all routes use it as buffer, otherwise it has conflicts
                    times_path++;
                    conflict_cells.insert(&c);
                    break;
                } else {
                    times_buffer++;
                }
    ...
    // std::unordered_set<Cell*> conflict_cells;
    // int conflict_size = 0;
    ...
    //     if (c.occupied_routes.size() > 1) {
    //         for (route_occupation occupied : c.occupied_routes) {
    //             if (!occupied.protected_ && conflict_cells.find(&c) == conflict_cells.end()) {
    //                 times_path++;
    //                 conflict_cells.insert(&c);
    //                 break;
    //             } else {
    ...
    //     for (int i = std::max(0, c.i - protection_length); i <= std::min(c.i + protection_length, grid_size[0] - 1); ++i) {
    //         for (int j = std::max(0, c.j - protection_length); j <= std::min(c.j + protection_length, grid_size[1] - 1); ++j) {
    //             for (int k = std::max(0, c.k - protection_length); k <= std::min(c.k + protection_length, grid_size[2] - 1); ++k) {
    //                 if (i == c.i && j == c.j && k == c.k) continue;
    //                 Cell* neighbor_cell = get_cell(i, j, k);
    //                 if (!neighbor_cell) continue;
    //                 if (conflict_cells.find(neighbor_cell) != conflict_cells.end()) continue;
    //                 if (!neighbor_cell->reserved) {
    //                     if (!neighbor_cell->occupied_routes.empty()) {
    //                         for (auto &occupied : neighbor_cell->occupied_routes) {
    //                             if (!occupied.protected_) {
    //                                 times_path++;
    //                                 conflict_cells.insert(neighbor_cell);
    //                                 break;
    //                             } else {
    //                                 times_buffer++;
    //                             }
    ...
    //     }
    //     if (times_path > 0) {
    //         conflict_size += times_path * 3 + times_buffer;
    //     }
    // }
    // return conflict_size;
}
double AStar_conops1::calculate_cell_distance(int i1, int j1, int k1, int i2, int j2, int k2, double grid_res) {
    double x1 = i1 * grid_res + grid_res / 2.0;
    double y1 = j1 * grid_res + grid_res / 2.0;
    double z1 = k1 * grid_res + grid_res / 2.0;
    double x2 = i2 * grid_res + grid_res / 2.0;
    double y2 = j2 * grid_res + grid_res / 2.0;
    double z2 = k2 * grid_res + grid_res / 2.0;
    return sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2) + pow(z1 - z2, 2));
}
std::vector<Conflict> AStar_conops1::conflict_cell_route_calculation(){
    std::vector<Conflict> conflicts;
    conflicts.clear();
    double conflict_threshold = 5.0; // 10 meters
    // 用于记录已经处理过的路径对
    std::set<std::pair<int, int>> processed_pairs;
    std::unordered_set<const Cell*> conflict_cells;
    ...
    for (size_t i = 0; i < all_paths.size() - 1; ++i) {
        for (size_t j = i + 1; j < all_paths.size(); ++j) {
            ...
            if (distance < conflict_threshold && l1 == l2 && conflict_cells.find(cell1) == conflict_cells.end()) { // 同一时间步长的点才会冲突
                processed_pairs.insert(route_pair);
                conflicts.push_back({cell1->i, cell1->j, cell1->k, l1,  std::vector<route_occupation>{route1}});
                // conflicts.push_back({cell2->i, cell2->j, cell2->k, l2,  std::vector<route_occupation>{route2}});
                conflict_cells.insert(cell1);
                // conflict_cells.insert(cell2);
    ...
    for (auto &c : cells){
        ...
        if(c.occupied_routes.size()>1){
            std::vector<route_occupation> conflicting_routes;
            for(route_occupation occupied:c.occupied_routes){
                if(occupied.protected_==false && conflict_cells.find(&c) == conflict_cells.end()){             //unless all routes use it as buffer, otherwise it has conflicts
    ...
    //         }
    //     }
    //     // 检查周围 protection_length 范围内的格子
    //     for (int i = std::max(0, c.i - protection_length); i <= std::min(c.i + protection_length, grid_size[0] - 1); ++i) {
    //         for (int j = std::max(0, c.j - protection_length); j <= std::min(c.j + protection_length, grid_size[1] - 1); ++j) {
    //             for (int k = std::max(0, c.k - protection_length); k <= std::min(c.k+ protection_length, grid_size[2] - 1); ++k) {
    //                 if (i == c.i && j == c.j && k == c.k) continue; // 跳过当前格子
    //                 Cell* neighbor_cell = get_cell(i, j, k);
    //                 if (conflict_cells.find(neighbor_cell) != conflict_cells.end()) continue; // 如果邻近格子已经记录为冲突则跳过
    ...
    //                             if (!occupied.protected_) {
    //                                 conflicts.push_back({i, j, k, 0, neighbor_cell->occupied_routes});
    //                                 conflict_cells.insert(neighbor_cell); // 记录邻近格子为冲突
    //                                 break; // 一旦检测到冲突即可，不必继续检查该格子的其他路线
    //                             }
    //                         }
    //                     }
    //                 }
...
std::vector<int> AStar_conops1::cal_conflicted_routes(Route p,int id){
...
double AStar_conops1::get_traversal_cost(const Cell *from, const Cell *to) {
    return distance3d(from, to) * (from->cost_coeff + to->cost_coeff) * 0.5;
}

double AStar_conops1::get_turning_cost(const Cell *from, const Cell *to) {
...
double AStar_conops1::get_climb_cost(const Cell *from, const Cell *to) {
...
double AStar_conops1::get_space_cost(const Cell *from, const Cell *to) {
    ...
    // std::cout << "grid_res: " << grid_res << std::endl;
    // space_length = std::ceil(10/grid_res);
...
double AStar_conops1::get_conflict_cost(const Cell *from, const Cell *to) {
    if (conflict_cost_coef <= 0.0) { return 0.0; }
    // std::cout << "get_conflict_cost: "  << std::endl;
    int protection_length = std::ceil(5.0 / grid_res);
    double count = 0.0;
    ...
    if(space_length==0){
        ...
        if (from->i==start->i and  from->j==start->j and from->k==start->k){
            ...
            //     if (get_cell(c0[0], c0[1], c0[2])->occupied_routes.empty()) {
            //         // 没有路线占用该单元格
            //     } else {
            //         // 多条路线占用该单元格
            ...
            // // 检查周围 protection_length 范围内的格子
            // for (int i = std::max(0, c0[0] - protection_length); i <= std::min(c0[0] + protection_length, grid_size[0] - 1); ++i) {
            //     for (int j = std::max(0, c0[1] - protection_length); j <= std::min(c0[1] + protection_length, grid_size[1] - 1); ++j) {
            //         for (int k = std::max(0, c0[2] - protection_length); k <= std::min(c0[2] + protection_length, grid_size[2] - 1); ++k) {
            //             if (i == c0[0] && j == c0[1] && k == c0[2]) continue; // 跳过当前格子
            //             if (!get_cell(i, j, k)->reserved) {
            //                 if (get_cell(i, j, k)->occupied_routes.empty()) {
            //                     // 没有路线占用该单元格
            //                 } else {
        ...
        while (c0[0] != to->i || c0[1] != to->j || c0[2] != to->k) {
            std::set<std::tuple<int, int, int>> cells_to_check;
            if (tMaxX < tMaxY) {
                ...
                // 记录 c0 以及周围 protection_length 范围内的格子
            for (int i = std::max(0, c0[0] - protection_length); i <= std::min(c0[0] + protection_length, grid_size[0] - 1); ++i) {
                for (int j = std::max(0, c0[1] - protection_length); j <= std::min(c0[1] + protection_length, grid_size[1] - 1); ++j) {
                    for (int k = std::max(0, c0[2] - protection_length); k <= std::min(c0[2] + protection_length, grid_size[2] - 1); ++k) {
                        cells_to_check.emplace(i, j, k);
                    }
                }
            }
    ...
    // space_length = protection_length;
    if (from->i==start->i and  from->j==start->j and from->k==start->k){
        for (int i = std::max(0, c0[0] - space_length); i < std::min(c0[0] + space_length + 1, grid_size[0]); i++) {
            for (int j = std::max(0, c0[1] - space_length); j < std::min(c0[1] + space_length + 1, grid_size[1]); j++) {
                for (int k = std::max(0, c0[2] - space_length); k < std::min(c0[2] + space_length + 1, grid_size[2]); k++) {
...
std::vector<Cell *> AStar_conops1::get_adjacent_cells(const Cell *c) {
    std::vector<Cell *> r;
...
std::vector<Cell *> AStar_conops1::get_adjacent_cells_subspace(const Cell *c) {
    std::vector<Cell *> r;
...
void AStar_conops1::update_cell(Cell *c, Cell *adj) {
    double g_old = adj->g;
    compute_cost(c, adj);
    if (adj->g < g_old) {
...
void AStar_conops1::compute_cost(Cell *c, Cell *adj) {
    double g_new = c->g;
...
void AStar_conops1::solve(array_t<int64_t> i1, array_t<int64_t> i2, std::promise<Route> & p) {
...
void AStar_conops1::solve_subspace(Agent agent, std::promise<Route> & p, std::promise<std::vector<int>> & 
...
double AStar_conops1::calculate_costs(Route r){
...
Route AStar_conops1::get_route(bool is_conflict_included=false) {
...
void backtrack_cell(const Route& p, const int& id,AStar_conops1 & solver, const int& space_length, const int& xlim, const int& ylim, const int& zlim, const double& grid_res)
{
    std::vector<std::vector<int32_t>> protection_zone_cells;
    protection_zone_cells.clear();
    for(size_t j=0;j<p.index.size()-1;j++){
...
void write_cells(AStar_conops1& solver,const std::vector<Agent>& route_parallel,const int& space_length, const int& xlim, const int& ylim, const int& zlim, const double& grid_res)
{
