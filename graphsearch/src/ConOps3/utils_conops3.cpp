...
const int& xlim, const int& ylim, const int& zlim, const double& grid_res)
{
    ...
    int c0[4] = {p0[0], p0[1], p0[2], p0[3]};
    results.route_cells.push_back({p0[0], p0[1], p0[2], p0[3]});
    ...
    for (int i = std::max(0, c0[0] - protection_length); i < std::min(c0[0] + protection_length + 1, xlim); i++) {
        for (int j = std::max(0, c0[1] - protection_length); j < std::min(c0[1] + protection_length + 1, ylim); j++) {
            for (int k = std::max(0, c0[2] - protection_length); k < std::min(c0[2] + protection_length + 1, zlim); k++) {
                results.protection_zone_cell.push_back({i, j, k, c0[3]});
            }
        }
    }
    ...
    while (c0[0] != p1[0] || c0[1] != p1[1] || c0[2] != p1[2]) {
        ...
        results.route_cells.push_back({c0[0], c0[1], c0[2], c0[3]});
    }
    return results;
}
...
{  
    ...
    oss.clear();
    oss<<"inv conflicts: ";
    for(auto& inv_conflict: inv_conflicts)oss<<'('<<inv_conflict.route_id1<<','<<inv_conflict.route_id2<<"), at time: "<< inv_conflict.l<<", ";
    LOG(INFO)<<oss.str();
}

WaypointAccInfo generate_traj_1d(double p, double max_v, double max_a, double t) {
    WaypointAccInfo hor_info;
    if(0.25*max_a*t*t<p){                           //current dynamic cannot satisfy 4d route temporal requirement, due to temporal window size
        LOG(ERROR)<<"cannot satisfy dyanmics";
        return hor_info;
    }else if(0.25*max_a*t*t>=p and 0.5*max_a*t>=max_v){     //三段式, 加速匀速减速
        double tx = (t-sqrt(t*t-4*p/max_a))/2;
        hor_info.t1 = tx;
        hor_info.a1 = max_a;
        hor_info.v1 = 0;
        hor_info.delta_s1 = 0.5 * max_a * tx * tx;

        hor_info.t2 = t-2*tx;
        hor_info.a2 = 0;
        hor_info.v2 = hor_info.v1 + hor_info.a1 * hor_info.t1;
        hor_info.delta_s2 = p-max_a*tx*(t-2*tx);

        hor_info.t3 = hor_info.t1;
        hor_info.a3 = -max_a;
        hor_info.v3 = max_a*tx;
        hor_info.delta_s3 = hor_info.delta_s1;
    }else if(0.25*max_a*t*t>=p and 0.5*max_a*t<max_v){      // 两段式
        double a=4*p/t/t;
        hor_info.t1 = t/2;
        hor_info.a1 = a;
        hor_info.v1 = 0;
